...
exports.getFormattedGameHistory = async (req, res) => {
    ...
    try {
        ...
        histories.forEach((record) => {
            ...
            // ✅ New Profit/Loss formula:
            // ProfitLoss = Total Spent - Total Income
            const totalSpent = totalBuyIn + totalRebuys + totalAddOns + totalDealerTips + mealsExp;
            const profitLoss = totalSpent - totalCashOut;

            grouped[s.id].totalProfitLoss += profitLoss;

            // Track overall totals
            if (profitLoss >= 0) totalLoss += profitLoss; // since positive means loss now
            else totalProfit += Math.abs(profitLoss); // negative means profit
        });
